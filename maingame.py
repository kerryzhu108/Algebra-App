...
running = True
past = -1  # check if sprite crosses equal sign
present = -1

selected = nums.numbers.sprites()[0]  # selected number
equal_sign = nums.equal_sign
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_z:
                ...
                nums.menu.menu(width, height, helpers.grey)
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if buttons.next_click_select:
                ...
                helpers.simplify(nums.numbers, buttons.select_box.init_pos, equal_sign)
                simplifiable = font.render(helpers.simplifiable[0], True, helpers.white, helpers.grey)  # updates error message
            ...
            for button in buttons.buttons_group:
                ...
                if button.rect.x - 10 < pygame.mouse.get_pos()[0] < button.rect.x + 60 \
                    ...
                    if button == buttons.answer:  # if click on answer button, remove all number sprites
                        helpers.store_info(nums.numbers, nums.bracket_nums, nums.history,
                                           nums.history_index, buttons.spawn_special_div_line)
                        helpers.remove_and_give_answer(equal_sign, nums.numbers, bedmas.answer)
        ...
        if past < equal_sign.rect.x <= present or present <= equal_sign.rect.x < past:
